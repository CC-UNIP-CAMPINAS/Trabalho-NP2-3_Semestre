...
public class Salvar {
	...
	public static void salvaRendimento() {
		for (Curso curso : Historico.cursos) {
			...
			try(BufferedWriter brRendimento = new BufferedWriter(new FileWriter(curso.toNomeArquivo()))) {
				if(arquivoRendimento.exists()) {
					arquivoRendimento.delete();
				}

				for (Rendimento rendimento : Historico.rendimentos) {
					if(curso.equals(rendimento.curso)) {
						brRendimento.write(rendimento.aluno.getId() + ";" + 
